<div class="w-full bg-white"><!----><!----><!---->
    <div>
                                    ...
                                    <div class="pb-3 md:pb-0 w-full text-xs">
                                        <div class="w-full">
                                            <div class="w-full overflow-x-scroll overflow-y-scroll">
                                                <table class="w-full table-auto" id="table_usulan_bahan_kerja">
                                                    <thead>
                                                        <tr><!---->
                                                            <th
                                                    ...
                                                    <tbody class="text-gray-600 text-sm font-light">
                                                        @foreach ($dataPerangkatKerja as $item)
                                                            <tr
                                                                ...
                                                                <td class="justify-start text-left cursor-auto py-2 px-2">
                                                                    <div class="flex item-right justify-end"><!----><!---->
                                                                        <span
                                                                            wire:click="editPerangkatKerja('{{ $item->id }}')"
                                                                            class="block px-1 py-1 text-xs text-gray-600 capitalize rounded cursor-pointer hover:bg-indigo-900 hover:text-gray-100 mr-2">
                                                                            Edit </span>
                                                                        <span
                                                                            wire:click="hapusPerangkatKerja('{{ $item->id }}')"
        ...
        <form wire:submit="{{ $isEditing ? 'updatePerangkatKerja' : 'insertPerangkatKerja' }}">
            <div class="w-full bg-white dark:bg-gray-800 pt-5 pb-4 sm:py-6 sm:pb-4">
                <h3 class="text-lg leading-6 font-medium text-gray-900 dark:text-white px-2 mb-3 pb-3">
                    {{ $isEditing ? 'Edit Perangkat Kerja' : 'Perangkat Kerja' }} </h3>
                <p class="text-xs text-red-500 px-2"> Perangkat kerja merupakan pedoman atau acuan yang digunakan untuk
                    memproses atau mengolah bahan kerja menjadi hasil kerja pada setiap uraian tugas. </p>
                <div class="block text-sm mb-3 mt-2"><label class="block text-sm px-2 mb-3 mt-2"><span
                            class="text-bkn-blue text-base">Uraian Perangkat Kerja <!----></span><textarea
                            wire:model="uraian_perangkat_kerja"
                            placeholder="Ketik Uraian Perangkat Kerja Disini (maks. 500 Karakter)" autocomplete="on"
                            rows="4"
                ...
                <div class="block text-sm mb-3 mt-2"><label class="block text-sm px-2 mb-3 mt-2"><span
                            class="text-bkn-blue text-base">Penggunaan Dalam Tugas <!----></span><textarea
                            wire:model="penggunaan"
                            placeholder="Ketik Penggunaan Dalam Tugas Disini (maks. 500 Karakter)" autocomplete="on"
                            rows="4"
                            class="block w-full border mt-1 px-2 rounded focus:border-indigo-800 focus:outline-none focus:shadow-outline-indigo form-textarea"></textarea></label><!---->
                </div>
                <div class="ml-2"><!----></div>
            ...
            <div class="flex">
                ...
                @if($isEditing)
                    <div class="w-1/4"><button type="button" wire:click="cancelEdit"
                            class="items-center float-right px-8 py-1 text-xs font-medium text-white border rounded bg-gray-500 border-gray-500 text-white">
                            BATAL </button></div>
                @else
                ...
                <div class="w-1/6"><button type="submit"
                        class="items-center float-right px-8 py-1 text-xs font-medium text-white border rounded bg-bkn-blue border-bkn-blue text-bkn-blue">
                        {{ $isEditing ? 'UPDATE' : 'SIMPAN' }} </button></div><!---->
            </div>
        </form>
    </div><!----><!----><!----><!----><!----><!----><!----><!----><!----><!---->
</div>