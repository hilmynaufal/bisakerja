...
<script>
    ...
    $('#chart-container').orgchart({
        ...
        'createNode': function($node, data) {
            // Sesuaikan lebar node berdasarkan panjang teks
            const nameLength = data.name.length;
            const minWidth = 120;
            const calculatedWidth = Math.min(Math.max(minWidth, nameLength * 8), 300);
            $node.find('.title').css('width', calculatedWidth + 'px');
            $node.find('.content').css('width', calculatedWidth + 'px');
    }
    });
</script>
@endscript