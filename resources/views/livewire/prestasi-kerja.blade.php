    ...
    <div><!---->
        <div class="w-full bg-white dark:bg-gray-800 pt-5 pb-4 sm:py-6 sm:pb-4">
            <h3 class="text-lg leading-6 font-medium text-gray-900 dark:text-white px-2 mb-3 pb-3">
                Prestasi Kerja Yang Diharapkan </h3>
            <div class="block text-sm px-2 mb-3 mt-2">
                <p class="text-bkn-blue text-base">Uraian Prestasi</p>
                @foreach ($dataPrestasiKerja as $item)
                    <form wire:submit.prevent="updatePrestasiKerja('{{ $item->id }}')">
                            ...
                            <div class="flex-initial w-11/12"><input type="text" wire:model="uraianModel.{{ $item->id }}"
                                ...
                                <div class="text-sm mb-3 flex">
                                    <div class="w-1/6"><button type="submit"
                                            ...
                                            UBAH </button></div>
                                    <button type="button" wire:click="hapusPrestasiKerja('{{ $item->id }}')"
                ...
                @endforeach
                <form wire:submit.prevent="insertPrestasiKerja">
                    <div class="flex flex-row mb-2">
                        <div class="flex-initial w-11/12"><input type="text" wire:model="uraian_prestasi_kerja"
                                placeholder="Ketik Prestasi Kerja Disini (maks. 500 Karakter)" autocomplete="on"
                                class="block w-full border text-sm rounded mt-1 px-2 focus:border-indigo-800 focus:outline-none focus:shadow-outline-indigo form-input"><!---->
                        </div>
