<div class="w-full bg-white px-2 pt-5 pb-4 sm:p-6 sm:pb-4"><!----><!----><!---->
    <div>
                                            ...
                                            <div class="w-full overflow-x-scroll overflow-y-scroll">
                                                <table class="w-full table-auto" id="table_usulan_bahan_kerja">
                                                    ...
                                                    <tbody class="text-gray-600 text-sm font-light">
                                                        @foreach ($dataReferensiPeraturan as $item)
                                                            <tr
                                                                ...
                                                                <td class="justify-start text-left cursor-auto py-2 px-2">
                                                                    @if($item->nama_file)
                                                                        <a href="{{ asset('peraturan/' . $item->nama_file) }}" 
                                                                           target="_blank" 
                                                                ...
                                                                <td class="justify-start text-left cursor-auto py-2 px-2">
                                                                    @if($item->waktu_upload)
                                                                        {{ \Carbon\Carbon::parse($item->waktu_upload)->format('d/m/Y H:i:s') }}
                                                                    @else
                                                                        -
                                                                    @endif
                                                                </td>
                                                                ...
                                                                <td class="justify-start text-left cursor-auto py-2 px-2">
                                                                    <div class="flex item-right justify-end"><!----><!---->
                                                                        ...
                                                                        @endif
                                                                        @if($id_jabatan === 'A8ACA73D811D3912E040640A040269BB')
                                                                            <span
                                                                                wire:click="hapusReferensiPeraturan('{{ $item->id }}')"
                                                                                class="block px-1 py-1 text-xs text-gray-600 capitalize rounded cursor-pointer hover:bg-indigo-900 hover:text-gray-100">
                                                                                Hapus 
                                                                            </span>
                                                                        @endif
        ...
        @if($id_jabatan === 'A8ACA73D811D3912E040640A040269BB')
        <form wire:submit="insertReferensiPeraturan">
            <div class="w-full bg-white dark:bg-gray-800 pt-5 pb-4 sm:py-6 sm:pb-4">
                <h3 class="text-lg leading-6 font-medium text-gray-900 dark:text-white px-2 mb-3 pb-3">
                    Referensi Peraturan </h3>
                <p class="text-xs text-red-500 px-2"> </p>
                <div class="block text-sm mb-3 mt-2"><label class="block text-sm px-2 mb-3 mt-2"><span
                            class="text-bkn-blue text-base">Nama Peraturan <!----></span><textarea
                            wire:model="nama_peraturan"
                            wire:ignore
                            placeholder="Ketik Uraian Bahan Kerja Disini (maks. 500 Karakter)" autocomplete="on"
                            rows="4"
                ...
                <div class="block text-sm mb-3 mt-2">
                    <label class="block text-sm px-2 mb-3 mt-2">
                        <span class="text-bkn-blue text-base">Upload File <!----></span>
                        <input wire:model="file_peraturan" type="file" 
                               accept=".pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx"
                               class="block w-full border mt-1 px-2 py-2 rounded focus:border-indigo-800 focus:outline-none focus:shadow-outline-indigo form-input">
                    </label>
                    <div class="px-2 text-xs text-gray-500">
                        Format yang didukung: PDF, DOC, DOCX, XLS, XLSX, PPT, PPTX (Maks. 50MB)
                    </div>
                </div>
                <!-- Loading bar -->
    <div wire:loading wire:target="file_peraturan" class="w-full bg-gray-200 rounded mt-2 h-2">
        <div class="h-full bg-indigo-600 animate-pulse rounded" style="width: 100%"></div>
    </div>

    <!-- Optional: Loading text -->
    <p wire:loading wire:target="file_peraturan" class="text-sm text-gray-600 mt-1">
