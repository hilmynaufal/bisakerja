...
class SyaratJabatan extends Component
{
    ...
    public function addBakatKerja($kode)
    {
        ...
        if (!$this->bakatKerja->contains('kode', $kode)) {
            if ($this->bakatKerja->count() < 3) {
                $this->bakatKerja->push($bakat);
            } else {
                LivewireAlert::title('Gagal!')
                    ->text('Maksimal 3 bakat kerja yang dapat dipilih!')
                    ->error()
                    ->toast()
                    ->position('top-end')
                    ->show();
    ...
    public function addTempramenKerja($kode)
    {
        ...
        if ($this->tempramenKerja->count() < 3) {
            ...
            LivewireAlert::title('Gagal!')
                ->text('Maksimal 3 tempramen kerja yang dapat dipilih!')
                ->error()
                ->toast()
                ->position('top-end')
                ->show();
    ...
    public function addMinatKerja($id)
    {
        ...
        if (!$this->minatKerja->contains('kode', $id)) {
            if ($this->minatKerja->count() < 3) {
                $minat->kode = $minat->id;
                $this->minatKerja->push($minat);
            } else {
                LivewireAlert::title('Gagal!')
                    ->text('Maksimal 3 minat kerja yang dapat dipilih!')
                    ->error()
                    ->toast()
                    ->position('top-end')
                    ->show();
    ...
    public function addUpayaFisik($id)
    {
        ...
        if (!$this->upayaFisik->contains('kode', $id)) {
            if ($this->upayaFisik->count() < 3) {
                $upaya->kode = $upaya->id;
                $this->upayaFisik->push($upaya);
            } else {
                LivewireAlert::title('Gagal!')
                    ->text('Maksimal 3 upaya fisik yang dapat dipilih!')
                    ->error()
                    ->toast()
                    ->position('top-end')
                    ->show();
    ...
    public function addFungsiPekerjaan($kode)
    {
        ...
        if (!$this->fungsiPekerjaan->contains('kode', $kode)) {
            if ($this->fungsiPekerjaan->count() < 3) {
                $this->fungsiPekerjaan->push($fungsi);
            } else {
                LivewireAlert::title('Gagal!')
                    ->text('Maksimal 3 fungsi pekerjaan yang dapat dipilih!')
                    ->error()
                    ->toast()
                    ->position('top-end')
                    ->show();
